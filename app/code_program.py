...
def check_answer_user(id_user_in_game, user_answer, count_question):
    ...
    if get_user.ok:
        data = get_user.json()
        if data["answer"]["correct_answer"] == False:
            print("Вы проиграли!!!")
            exit(0)
        if data['answer']['last_block'] is True:
            return 0
...
def get_question(id_user_in_game, count_question):
    ...
    if get_user.ok:
        data = get_user.json()
        print(f"Вопрос №{count_question}: {data['answer']['question']}")
        print("-" * 25)
        print(f"Ответ №1: {data['answer']['answers'][0]}")
        print(f"Ответ №2: {data['answer']['answers'][1]}")
        print(f"Ответ №3: {data['answer']['answers'][2]}")
        print(f"Ответ №4: {data['answer']['answers'][3]}")
        print()
        print("Если хотите подсказку введите: -1")
        print("-" * 25)
        user_answer_to(id_user_in_game, count_question)
        count_question += 1
